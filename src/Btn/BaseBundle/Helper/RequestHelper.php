...
class RequestHelper
{
    ...
    public static function isAsseticRequest(Request $request)
    {
        ...
        $isAssetic = (
            $attr->get('_controller') === 'assetic.controller:render'
            || substr($attr->get('_route'), 0, 8) === '_assetic'
        ) ? true : false;
    ...
    public static function isToolbarRequest(Request $request)
    {
        $attr = $request->attributes;
        $isToolbar = (
            $attr->get('_controller') == 'web_profiler.controller.profiler:toolbarAction'
            || $attr->get('_route') == '_wdt'
        ) ? true : false;
    ...
    public static function isProfilerRequest(Request $request)
    {
        $attr = $request->attributes;
        $isProfiler = (
            $attr->get('_controller') == 'web_profiler.controller.profiler:panelAction'
            || $attr->get('_route') == '_profiler'
        ) ? true : false;
